...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create client-7d6c
  fly apps create client-7d6c-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app client-7d6c
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app client-7d6c-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'client-7d6c-staging' does not match app name in config file 'client-7d6c'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name client-7d6c-db
  fly postgres attach --app client-7d6c client-7d6c-db

  fly postgres create --name client-7d6c-staging-db
  fly postgres attach --app client-7d6c-staging client-7d6c-staging-db
  ```
